...
#include "EngineEditorWindow.h"
 ...
 bool UEngineEditorGUI::show_demo_window = true;
 bool UEngineEditorGUI::show_another_window = false;
 ImVec4 UEngineEditorGUI::clear_color = ImVec4(0.45f, 0.55f, 0.60f, 1.00f);
...
void UEngineEditorGUI::GUIRender(Scene* _scene, float _DeltaTime)
{
    ...
    for (std::pair<const std::string, std::shared_ptr<UEngineEditorWindow>> WindowPair : EditorWindows)
    {
        ...
        Window->Update(_scene, _DeltaTime);

        if (true == Window->IsActive)
        {
            Window->Begin();

            Window->OnGui(_scene, _DeltaTime);
...
bool UEngineEditorGUI::CreateDeviceD3D(HWND hWnd)
{
    // Setup swap chain
...
void UEngineEditorGUI::CleanupDeviceD3D()
{
    CleanupRenderTarget();
...
void UEngineEditorGUI::CreateRenderTarget()
{
    for (UINT i = 0; i < NUM_BACK_BUFFERS; i++)
    {
...
void UEngineEditorGUI::CleanupRenderTarget()
{
    WaitForLastSubmittedFrame();
...
void UEngineEditorGUI::WaitForLastSubmittedFrame()
{
...
FrameContext* UEngineEditorGUI::WaitForNextFrameResources()
{
...
LRESULT WINAPI UEngineEditorGUI::WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
